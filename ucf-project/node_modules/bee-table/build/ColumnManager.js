...
var ColumnManager = function () {
  ...
  ColumnManager.prototype.showLeafColumns = function showLeafColumns(fixed) {
    var columns = [];
    if (fixed) {
      if (fixed == 'right') {
    ...
    var showColumns = columns.filter(function (col) {
      return col.ifshow == true;
    });
    return showColumns;
  };
  ...
  ColumnManager.prototype.clearCache = function clearCache() {
    this._cached = {};
  };
  ...
  ColumnManager.prototype.getColumnWidth = function getColumnWidth(contentWidth) {
    ...
    var centerShowColCount = 0; //中间区域的可见列个数
  ...
  ColumnManager.prototype.getLeftColumnKeys = function getLeftColumnKeys() {
    var _this15 = this;

    return this._cache('leftColumnKeys', function () {
      return _this15.leftColumns().map(function (column) {
        return column.key || column.dataIndex;
      });
    });
  };

  ColumnManager.prototype.getRightColumnKeys = function getRightColumnKeys() {
    var _this16 = this;

    return this._cache('rightColumnKeys', function () {
      return _this16.rightColumns().map(function (column) {
        return column.key || column.dataIndex;
      });
    });
  };
  ...
  ColumnManager.prototype.findColumn = function findColumn(columnKey) {
    return this._findColumn(columnKey, this.columns || []);
  };

  ColumnManager.prototype._findColumn = function _findColumn(columnKey, columns) {
    for (var i = 0; i < columns.length; i++) {
      var findOne = null;
      var column = columns[i];
      if (!column) continue;
      if (column.key == columnKey || column.dataIndex == columnKey) {
        //兼容只有key的情况
        findOne = column;
      } else {
        if (column.children) {
  ...
  ColumnManager.prototype._leafColumns = function _leafColumns(columns) {
    ...
    columns.forEach(function (column) {
      if (!column.children) {
        ...
        if (!_this17.originWidth) {
          defaultOpt.width = ColumnManager.DefaultColumnWidth;
        }
        var newColumn = _extends({}, defaultOpt, column);
        leafColumns.push(newColumn);
      } else {
        leafColumns.push.apply(leafColumns, _toConsumableArray(_this17._leafColumns(column.children)));
      }
    });
    return leafColumns;
  };
...
ColumnManager.DefaultColumnWidth = 200;
