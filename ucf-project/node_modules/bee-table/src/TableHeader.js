...
class TableHeader extends Component {
  ...
  //----交换列头顺序时抓起的副本---begin
  createTableNone=()=>{
    this._table_none_cont_id = this.props.tableUid || "_table_uid_"+new Date().getTime();
    if(document.getElementById(this._table_none_cont_id))return;
    let div = document.createElement("div");
    div.className = `${prefix}-table-drag-hidden-cont`;
    div.id = this._table_none_cont_id;
    document.body.appendChild(div);
    return div;
  }
  destroyTableNone=()=>{
    let elem = document.getElementById(this._table_none_cont_id);
    if(!elem)return;
    if(elem.remove){
      elem.remove();
   ...
   */
  dragBorderEvent (){
    if(!this.table||!this.table.thDragGaps)return{off:()=>{},on:()=>{}};
  ...
  */
  ...
  onTrMouseMove = (e) => {
    ...
    if(!this.props.dragborder && !this.props.draggable)return;
    const { clsPrefix ,dragborder,contentDomWidth,scrollbarWidth,contentTable,headerScroll,lastShowIndex,onMouseMove,onDraggingBorder, leftFixedWidth, rightFixedWidth, bodyDisplayInRow, eventNoStop} = this.props;
    !eventNoStop && Event.stopPropagation(e);
    let event = Event.getEvent(e);
  ...
  */
   ...
   */
  dragAbleEvent (){
    if (!this.table||!this.table.tr)return{off:()=>{},on:()=>{}};
    let  events = [
  ...
  onDragStart = (e) => {
    ...
    let event = Event.getEvent(e);
    let target = this.drag.dragTarget;//抓起的th目标
    let currColKey = target.getAttribute("data-col-key");
    let currColIndex = target.getAttribute('data-col-index');
  ...
  onDrop = (e) => {
    ...
    let dragSourceColumn = this.props.rows[0].find(da=>da.key == dragColKey);//抓取来源列定义
    let dragTargetColumn = this.props.rows[0].find(da=>da.key == dropColKey);//放置目标列定义
...
//---拖拽列交换----end-----
  ...
  render() {
    const { clsPrefix, rowStyle,draggable,tableUid,
        dragborder, rows,filterable,fixed,columnsChildrenList,columnManager
    } = this.props;
    let attr = dragborder ? { id: `${prefix}-table-drag-thead-${this.theadKey}` } : {};
    let showColumns = columnManager.showLeafColumns(fixed);
    return (
          ...
          return(<tr key={index} style={rowStyle} className={(filterable && index == rows.length - 1)?'filterable':''}>
            {row.map((da, columIndex, arr) => {
              ...
              */
              ...
              if(dragborder && da.dragborder != 'disabled' && !da.colSpan) {//如果存在colSpan则表明其是组合列（不允许拖拽），不是最终显示列。
                let isLastCol = false;//是否为本区域最后一列
                if(fixed && da.fixed){
              ...
              if(!da.fixed && !fixed){ //非固定表头 , data-type="draggable"标识允许进行列拖拽排序
                return (<th {...thProps} key={colKey+"_"+fixed} className={thClassName} data-type="draggable" {...domData} onCopy={(event) => {this.onCopy(da,columIndex,event)}}>
                    {da.children}
                    {dragGapView}
                </th>)
              }else if(da.fixed && fixed){ //固定表头
                if(da.dataIndex =='checkbox' && da.fixed=='left') {//多选框列
                  return <th {...thProps}>{da.children}</th>;
                }else if(filterable && index == rows.length - 1){//固定列下的筛选单元格需要一个空占位
                  return <th dangerouslySetInnerHTML={{__html:'&nbsp;'}}></th>
                }else{
                  return (<th {...thProps} onClick={da.onClick?da.onClick.bind(this,da):null} key={colKey+"_"+fixed} {...domData} onCopy={this.onCopy}>
                     {da.children}
                     {dragGapView}
                  </th>)
                }
