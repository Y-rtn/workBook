...
var Tree = function (_React$Component) {
  ...
  Tree.prototype.onCheck = function onCheck(treeNode, event) {
    var _this4 = this;
    ...
    var oldCheckedKeys = this.state.checkedKeys;
    if (this.props.checkStrictly) {
      ...
      if (!('checkedKeys' in this.props)) {
        if (!checked) {
          oldCheckedKeys = oldCheckedKeys.filter(function (oldCheckedKey) {
            // 旧的已选的数据如果被取消勾选，从旧的已选数据种去掉
            return oldCheckedKey !== key && checkedKeys.includes(key);
          });
        }
        this.setState({
          checkedKeys: lazyLoad ? [].concat(_toConsumableArray(oldCheckedKeys), _toConsumableArray(checkedKeys)) : checkedKeys
        });
      }
  ...
  Tree.prototype.onSelect = function onSelect(treeNode) {
    ...
    if (selectedKeys.length) {
      var treeNodes = this.props.children || (treeNode.props.root ? treeNode.props.root.cacheTreeNodes : this.cacheTreeNodes); // 添加this.cacheTreeNodes，获取树节点数据(当从onDown或onUp进入时treeNode.props不存在root属性)
  ...
  Tree.prototype.goDown = function goDown(currentPos, currentIndex, e, treeNode) {
    ...
    }, undefined, this.startIndex); // 当懒加载是传数据初始值计算
    ...
    //查询的下一个节点不为空的话，则选中
    if (nextTreeNode) {
      var queryInfo = 'span[pos="' + nextPos + '"]';
    ...
    if (prevTreeNode) {
      if (preIndex >= 0) {
        ...
        if (state.expandedKeys.indexOf(prevTreeNode.key) > -1) {
          var preElementArr = e.target.parentElement.previousElementSibling.querySelectorAll('span[pos]');
          ...
          //上一个节点没有展开
          preElement = e.target.parentElement.previousElementSibling.querySelector('span[pos]');
        }
      } else {
        // 不存在上一个节点时，选中它的父节点
        preElement = e.target.parentElement.parentElement.parentElement.querySelector('span[pos]');
      }
    } else {
      this._setDataTransfer(e);
